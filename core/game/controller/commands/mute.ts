...
export function cmdMute(byPlayer: PlayerObject, message: string[]): void {
    if(window.gameRoom.playerList.get(byPlayer.id)!.admin == true) {
        if(message !== undefined && message[1].charAt(0) == "#") {
            let target: number = parseInt(message[1].substr(1), 10);
            if(isNaN(target) != true && window.gameRoom.playerList.has(target) == true) {
                ...
                if(window.gameRoom.playerList.get(target)!.permissions.mute === true) {
                    ...
                    window.gameRoom._room.sendAnnouncement(Tst.maketext(LangRes.command.mute.successUnmute, placeholder), null, 0x426AD6, "normal", 1);
                } else {
                    ...
                    if(window.gameRoom.config.settings.antiMuteAbusing === true) { //when anti abusing option is enabled, check the condition by compare time
                        if(mutedTimeStamp > window.gameRoom.playerList.get(target)!.permissions.muteExpire + window.gameRoom.config.settings.muteAllowIntervalMillisecs) {
                            ...
                            window.gameRoom._room.sendAnnouncement(Tst.maketext(LangRes.command.mute.successMute, placeholder), null, 0x426AD6, "normal", 1);
                        } else {
                            // if mute again in too short time
                            window.gameRoom._room.sendAnnouncement(Tst.maketext(LangRes.command.mute.muteAbusingWarn, placeholder), byPlayer.id, 0x426AD6, "normal", 2);
                        }
                ...
                window.gameRoom._room.sendAnnouncement(LangRes.command.mute._ErrorNoPlayer, byPlayer.id, 0x426AD6, "normal", 2);
            }
        } else {
            window.gameRoom._room.sendAnnouncement(LangRes.command.mute._ErrorNoPlayer, byPlayer.id, 0x426AD6, "normal", 2);
        }
    } else {
        window.gameRoom._room.sendAnnouncement(LangRes.command.mute._ErrorNoPermission, byPlayer.id, 0x426AD6, "normal", 2);
    }
}
